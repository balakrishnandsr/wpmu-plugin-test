...
class Auth extends Base {
	...
	private $redirect_url = 'http://localhost/wp-json/wpmudev/v1/auth/confirm';
	...
	public function set_up( string $client_id = '', string $client_secret = '' ): bool {
		...
		$redirect_url  = ! empty( $this->redirect_url ) ? $this->redirect_url : 'http://localhost/wp-json/wpmudev/v1/auth/confirm';
	...
	protected function get_settings() {
		...
		if ( is_null( $settings ) ) {
			$settings = get_option( WPMUDEV_PLUGINTEST_SETTINGS );
		}
